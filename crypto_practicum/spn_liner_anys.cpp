...
unsigned int liner_anys(unsigned int *plains, unsigned int *ciphers, int num)
{
    ...
    // 提前线性运算存储结果
    unsigned long long z1_2[16][125]; // x_5 ^ x_7 ^ x_8 ^ u_6 ^ u_8;
    unsigned long long z1_4[16][125]; // u_14 ^ u_16;
    unsigned long long z2_1[16][125]; // u_2 ^ u_4;
    unsigned long long z2_2[16][125]; // x_5 ^ x_6 ^ u_6 ^ u_8;
    unsigned long long z2_3[16][125]; // u_10 ^ u_12;
    // timer_start();
    for (int i = 0; i < 16; i++)
    {
        int k1 = i;
        ...
        // 下标和移位
        int index = -1, shift = 0;
        unsigned long long t1_2 = 0, t1_4 = 0, t2_1 = 0, t2_2 = 0, t2_3 = 0;
        for (int j = 0; j < num; j++)
        {
            ...
            // 计算随机变量
            t1_2 = (t1_2 << 1) | (x_5 ^ x_7 ^ x_8 ^ u_6 ^ u_8);
            t1_4 = (t1_4 << 1) | (u_14 ^ u_16);

            t2_1 = (t2_1 << 1) | (u_2 ^ u_4);
            t2_2 = (t2_2 << 1) | (x_5 ^ x_6 ^ u_6 ^ u_8);
            t2_3 = (t2_3 << 1) | (u_10 ^ u_12);
            shift++;
            if (shift == 64)
            {
                shift = 0;
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        int k2 = i >> 4;
        int k4 = i & 0xf;
        unsigned long long x_or = 0;
        for (int j = 0; j < num / 64; j++)
        {
            x_or = z1_2[k2][j] ^ z1_4[k4][j];
            cnt += num_one_bit[x_or & 0xffff];
            cnt += num_one_bit[(x_or >> 16) & 0xffff];
            cnt += num_one_bit[(x_or >> 32) & 0xffff];
            cnt += num_one_bit[(x_or >> 48) & 0xffff];
        }
        cnt = cnt - num / 2;
        if (cnt < 0)
        {
    ...
    for (int k = 0; k < save_max; k++)
    {
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            for (int j = 0; j < num / 64; j++)
            {
                x_or = z2_1[k1][j] ^ z2_2[k2][j] ^ z2_3[k3][j];
                cnt += num_one_bit[x_or & 0xffff];
                cnt += num_one_bit[(x_or >> 16) & 0xffff];
                cnt += num_one_bit[(x_or >> 32) & 0xffff];
                cnt += num_one_bit[(x_or >> 48) & 0xffff];
