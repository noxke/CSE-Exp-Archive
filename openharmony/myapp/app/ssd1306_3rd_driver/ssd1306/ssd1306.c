...
#include "iot_i2c.h"    // OpenHarmony API：IoT硬件设备操作接口-I2C
#include "iot_errno.h"  // OpenHarmony API：IoT硬件设备操作接口-错误代码定义
...
#define SSD1306_I2C_BAUDRATE 400 * 1000
...
{
    // 计算每个tick用多少ms
    // osKernelGetTickFreq()：获取内核tick频率（每秒的tick数），100。
    // 1000/100=10ms
    uint32_t msPerTick = 1000 / osKernelGetTickFreq();

    // 计算内核tick了多少ms
    // osKernelGetTickCount()：获取内核当前tick数。
    uint32_t tickMs = osKernelGetTickCount() * msPerTick;

    // 计算每ms产生多少系统时钟
    // osKernelGetSysTimerFreq()：获取系统时钟频率（每秒的系统时钟数），160000000，即160M。
    // 160M/1000=160K cycles/ms
    uint32_t csPerMs = osKernelGetSysTimerFreq() / 1000;

    // 计算每个tick产生多少系统时钟
    // 160K*10=1600K cycles/tick
    uint32_t csPerTick = csPerMs * msPerTick;

    // 计算剩余不足1个tick的ms数
    // osKernelGetSysTimerCount()：获取系统当前时钟数。
...
void ssd1306_Init(void)
{
    ...
    IoTI2cInit(SSD1306_I2C_IDX, SSD1306_I2C_BAUDRATE);
    usleep(20 * 1000);
...
// Write full string to screenbuffer and update screen afterwards
char ssd1306_PrintString(char *str, FontDef Font, SSD1306_COLOR color)
{
    ssd1306_DrawString(str, Font, color);
    ssd1306_UpdateScreen();
}
...
void ssd1306_DrawRegion(uint8_t x, uint8_t y, uint8_t w, uint8_t h, const uint8_t *data, uint32_t size, uint32_t stride)
{
    ...
    // 计算要显示多少行
    // 将按字节计算的内容大小，转换为按像素计算的内容大小，再除以内容宽度，得到行数
