...
Specifier:
    ...
    | StructSpecifier {$$=new NSpecifier(); $$->line=yylineno;}   
    ;

StructSpecifier:
    STRUCT OptTag LC DefList RC {$$=new NStructSpecifier($2, $4); $$->line=yylineno;}   
    | STRUCT Tag {$$=new NStructSpecifier($2); $$->line=yylineno;}   
    ;
...
/*Statements*/
CompSt:
    LC DefList StmtList RC {$$=new NCompSt($2, $3); $$->line=$2->line-1;} // 这里应该拿LC的行号，但是拿不到
    ;
StmtList: {$$=nullptr;}  
    | Stmt StmtList  {$$=new NStmtList(*$1, $2); $$->line=$1->line;}
    ;
Stmt:
    Exp SEMI {$$=new NExpStmt(*$1); $$->line=yylineno;}
    | CompSt {$$=new NCompStStmt(*$1); $$->line=yylineno;}      //复合语句结点直接最为语句结点，不再生成新的结点
