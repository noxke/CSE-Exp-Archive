...
//% type  用于定义非终结符的语义值类型
%type  <p> Program ExtDefList ExtDef Specifier ExtDecList DecList VarDec FunDec CompSt DefList VarList ParamDec Dec Def StmtList Exp ExpEnd Stmt Args OptTag  Tag StructSpecifier
//% token 用于定义终结符的语义值类型
%token <type_int> INT                   //指定INT的语义值是type_int，由词法分析得到的数值
%nonassoc <type_id> ID RELOP TYPE STRUCT   //指定ID,RELOP 的语义值是type_id，由词法分析得到的标识符字符串mmecpy得到的
%token <type_float> FLOAT               //指定ID的语义值是type_float，由词法分析得到的float
%token <type_char> CHAR
%left LP RP LC RC LB RB SEMI COMMA DOT     //用bison对该文件编译时，带参数-d，生成的exp.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码
%left PLUS MINUS STAR DIV MOD ASSIGNOP AND OR NOT IF BREAK ELSE WHILE RETURN PLUSASS MINUSASS STARASS DIVASS MODASS PLUSPLUS MINUSMINUS
...
Program: 
    ExtDefList                  {std::cout<<"Program"<<std::endl;} /*显示规则对应非终结符*/
    ;

ExtDefList:                      {std::cout<<"ExtDefList"<<std::endl;}
...
OptTag:                         {std::cout<<"OptTag"<<std::endl;}
...
StmtList:                       {std::cout<<"StmtList"<<std::endl;}
...
/*Local Definitions*/
DefList:                        {std::cout<<"DefList"<<std::endl;}
...
Exp:
    ...
    |ExpEnd AND Exp                {std::cout<<"Exp"<<std::endl;}
    |ExpEnd OR Exp                 {std::cout<<"Exp"<<std::endl;}
    |ExpEnd RELOP Exp              {std::cout<<"Exp"<<std::endl;}
    |ExpEnd PLUS Exp               {std::cout<<"Exp"<<std::endl;}
    |ExpEnd MINUS Exp              {std::cout<<"Exp"<<std::endl;}
    |ExpEnd STAR Exp               {std::cout<<"Exp"<<std::endl;}
    |ExpEnd DIV Exp                {std::cout<<"Exp"<<std::endl;}
