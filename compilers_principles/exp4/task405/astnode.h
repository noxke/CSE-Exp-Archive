...
public:
    int layer = -1;
    std::vector<std::map<std::string, Value *>> vecValues;
    // 添加新一层
    void newLayer()
    {
        layer++;
        std::map<std::string, Value *> newLayer;
        vecValues.push_back(newLayer);
    }
    // 返回上一层
    void backLayer()
    {
    ...
    // 添加变量
    bool pushValue(std::string name, Value *value)
    {
        if (layer < 0)
        {
            return false;
        }
        if (vecValues[layer][name])
        {
            // 变量已存在
            return false;
        }
        vecValues[layer][name] = value;
        return true;
    }
    // 查找变量
